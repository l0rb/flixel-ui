...
class FlxUI extends FlxUIGroup implements IEventGetter
{
	...
	public function new(data:Access = null, ptr:IEventGetter = null, superIndex_:FlxUI = null, tongue_:IFireTongue = null, liveFilePath_:String = "",
			uiVars_:Map<String, String> = null, loadThingCallback_:IFlxUIWidget->String->Void = null, postLoadThingCallback_:IFlxUIWidget->String->Void = null)
	{
		super();
	...
	@:access(Xml)
	private function resolveInjections(data:Access):Access
	{
	...
	private function _loadThingGetInfo(data:Access):Access
	{
		...
		var definition:Access = getUseDef(info);
	...
	private function _loadThing(type:String, data:Access):IFlxUIWidget
	{
		...
		{
			case "region":
				returnThing = _loadRegion(info);
			case "chrome", "nineslicesprite", "nine_slice_sprite", "nineslice", "nine_slice":
				returnThing = _load9SliceSprite(info);
			case "tile_test":
				returnThing = _loadTileTest(info);
			case "line":
				returnThing = _loadLine(info);
			case "box":
				returnThing = _loadBox(info);
			case "sprite":
				returnThing = _loadSprite(info);
			case "bar":
				returnThing = _loadBar(info);
			case "text":
				returnThing = _loadText(info, false); // if input has events
			case "text_region":
				returnThing = _loadText(info, true);
			case "input_text":
				returnThing = _loadInputText(info); // if input has events
			case "numstepper", "num_stepper", "numeric_stepper":
				returnThing = _loadNumericStepper(info); // has events, params
			case "button":
				returnThing = _loadButton(info); // has events, params
			case "button_toggle":
				returnThing = _loadButton(info, true, true); // has events, params

			case "tab_menu":
				returnThing = _loadTabMenu(info); // has events, params?

			case "dropdown_menu", "dropdown", "pulldown", "pulldown_menu":
				returnThing = _loadDropDownMenu(info); // has events, params?

			case "checkbox":
				returnThing = _loadCheckBox(info); // has events, params
			case "radio_group":
				returnThing = _loadRadioGroup(info); // has events, params
			case "layout", "ui":
				returnThing = _loadLayout(info);
			case "failure":
				if (_failure_checks == null)
				{
			...
			case "show", "hide":
				...
				if (activeStatus != null)
					activeStatus = U.xml_bool(info.x, "active");
	...
	@:access(flixel.text.FlxText)
	private function _loadText(data:Access, asRegion:Bool = false):IFlxUIWidget
	{
	...
	private function _loadTest(data:Access, nodeName:String = "load_if"):Bool
	{
	...
	private function newUI(data:Access = null, ptr:IEventGetter = null, superIndex_:FlxUI = null, tongue_:IFireTongue = null, liveFilePath_:String = "",
			uiVars_:Map<String, String> = null, loadThingCallback_:IFlxUIWidget->String->Void = null,
			postLoadThingCallback_:IFlxUIWidget->String->Void = null):FlxUI
	{
	...
	{
		var back_def:Access = getUseDef(data, "back_def");
	...
	public function load9SliceSprite_scaleSub(slice9:Array<Int>, data:Access, src:String, srcString:String = "src"):Array<Int>
	{
	...
	private function _loadBox(data:Access):FlxUIGroup
	{
		var src:String = "";
			...
			else // If Width or Height or both is/are supplied, do some scaling
			{
				...
				if (!scalable)
				{
					...
					if (isAnimated)
					{
						...
						if (testAsset == null)
							testAsset = Assets.getBitmapData(src);
					...
					if (sw != 0 && sh != 0)
					{
						if (tilesTall > 1 || tilesWide > 1)
						{
							...
							var str = U.scaleAndStoreTileset(U.gfx(srcSuffix), scale_y, Std.int(testAsset.width / tilesWide),
								Std.int(testAsset.height / tilesTall), Std.int(sw / tilesWide), Std.int(sh / tilesTall), smooth);
	...
	private function getUseDef(node:Access, attName:String = "use_def")
	{
	...
	private function formatButtonText(data:Access, button:Dynamic):FlxText
	{
		if (data != null && data.hasNode.text)
		{
			...
			var text_def:Access = getUseDef(textNode);
