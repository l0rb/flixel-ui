...
class U
{
	...
	private static function isStrPerc(str:String):Bool
	{
		...
		if (!hasLeadingNums && !hasTrailingNums)
			return false;
	...
	public static function isFormula(str:String):Bool
	{
		...
		for (i in 0...len)
		{
			...
			else if ((code >= zero && code <= nine) || (code >= A && code <= Z) || (code >= a && code <= z) || (code == underscore))
			{
				if (!hasPeriod)
				{
			...
			{
				if (hasLeadingChars && hasPeriod && hasTrailingChars)
					return true;
