...
class FlxUICursor extends FlxUISprite
{
	...
	{
		if (bloc >= aloc && bloc <= aloc + asize)
			return true;
		if (aloc >= bloc && aloc <= bloc + bsize)
			return true;
		if (bloc + bsize >= aloc && bloc + bsize <= aloc + asize)
			return true;
		if (aloc + asize >= bloc && aloc + asize <= bloc + bsize)
			return true;
	...
	private function _wrapFind(X:Int, Y:Int, indexValue:Int, listWidget:Array<IFlxUIWidget>, listLists:Array<WidgetList>):Int
	{
		...
		for (i in 0...length)
		{
			if (i != location)
			{
				...
				{
					if (FlxMath.sameSign(dx, X) == false && dx != 0)
					{ // I want the WRONG direction this time
						dx = Math.abs(dx);
						if (dx > bestdx)
						{
	...
	private function _doInput(X:Int, Y:Int, recursion:Int = 0):Void
	{
		...
		if (Y == 0) // horizontal movement: can be either linear or grid
		{
			...
			{
				case LINEAR:
					location = _doInputLinear(X);
				case GRID:
					location = _doInputGrid(X, 0);
			}
